...
def prompt(cap_start: float, threshold: float, util_pct_max: float,
    ...
    while portfolio < threshold:
        ...
        if level <= capacity:
            if (contract_amt + 1) <= contract_limit:
        ...
        def mathing(portfolio, contract_amt) -> tuple[
            ...
            """
            Determine's the ending balance and tests if it exceeds the
            provided monetary threshold.
...
def main(os_type: str) -> None:
    ...
    contract_limit = get_input("What's the maximum amount of contracts you want to trade?", 999999, int)
    compound_amt = get_input("How much is your typical contribution?", 500, float)
    compount_cycle = get_input("How many days is inbetween each contribution", 30, int)
